...
document.getElementById("startButton").addEventListener('click', startOnClick);
...
/*
// quick sort works (tested by logging cube heights) but javascript async nature makes the mixer
// animation happen alongside the sorting algorithm working. Will continue to find something that works
*/
const sleep = (time) => {
  return new Promise(resolve => setTimeout(resolve, time))
}

//STARTING THE SORT W/ A SELECTION (make better later)
function startOnClick() {
  let sortSelected = document.getElementById("sortSelect");
  if(sortSelected.value == "noOption") alert("Please pick an algorith");
  else if(sortSelected.value == "quickSort") {
    quickSort(cube, 0, cube.length-1);
  }
  console.log("done");
}

//QUICK SORT STUFF LINES 230-254
async function partition(cube, low, high) {
  //pivot
  let piv = cube[high].geometry.parameters.height;

  let i = (low - 1);

  for(let j = low; j <= high - 1; j++) {
    if(cube[j].geometry.parameters.height < piv) {
      i++;

      mixers = swap(cube, i, j);
      mixer1 = mixers[0];
      mixer2 = mixers[1];
      swapAnimation();
      await sleep(7000);
...
async function quickSort(cube, low, high) {
  if(low < high) {
    let partIndex = await partition(cube, low, high);

    quickSort(cube, low, partIndex - 1);
    quickSort(cube, partIndex + 1, high)
  }
}
